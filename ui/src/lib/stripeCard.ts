 ...
 *
 * Pure so it has a spec. The API decides who may connect (owner only); this
 * only decides whether to draw the button, which is a courtesy, not a
 * permission.
 */
...
export interface StripeCard {
  state: 'not_connected' | 'pending' | 'enabled'
  title: string
  text: string
